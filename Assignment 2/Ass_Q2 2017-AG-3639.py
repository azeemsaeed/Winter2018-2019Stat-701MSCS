...
#Yield
X-1 = np.array([100, 200, 300, 400, 500, 600, 700])
#Fertilizer
X-2 = np.array([10, 20, 10, 30, 20, 20, 30])
#Rainfall
#Observations
n = Y.size,
print("\n")
...
# Mean Calculations
Y_mean =  Sum(Y)/Y.size
X-1 _mean =  Sum(X-1)/X-1.size
X-2 _mean =  Sum(X-2)/X-2.size

print("\n")
print("Mean of Yield = ",Y_mean)
print("Mean of Fertilizer = ",X-1_mean)
print("Mean of Rainfall = ",X-2_mean)
...
ax.scatter(
    data_frame["Fertilizers"]
    , data_frame["Rainfall"]
    , data_frame["Yield"]
    , color="Red"
    , marker="o"
    , alpha=5
)ax.set_xlabel("Fertilizers")
